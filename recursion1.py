...
#  Description: asignment 9 recursion practice
...
def strDist(str, sub):
    len_str = len(str)
    len_sub = len(sub)
    diff = int(len_str - len_sub)
    if len_str < len_sub:
        return 0
    if str[0:len_sub] == sub:
        if str[diff: len_str] == sub:
            return len_str
        return strDist(str[0:len_str - 1], sub)
    else:
