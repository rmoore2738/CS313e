...
def splitArrayHelper(nums, i, j):
    if i == len(nums):
        return(j == 0)
    if splitArrayHelper(nums, i + 1, j + nums[i]):
        return True
    else:
        return splitArrayHelper(nums, i + 1, j - nums[i])
...
def splitOdd10Helper(nums, i, j, k):
    if i == len(nums):
        return((j % 2 == 1 and k % 10 == 0)) or (k % 2 == 1 and j % 10 == 0)
    if splitOdd10Helper(nums, i + 1, j + nums[i], k):
        return True
    else:
        return splitOdd10Helper(nums, i + 1, j, k + nums[i])
...
def split53Helper(nums, i, j):
    if i == len(nums):
        return j == 0
    if nums[i] % 5 == 0:
        return split53Helper(nums, i + 1, j + nums[i])
    if nums[i] % 3 == 0:
        return split53Helper(nums, i + 1, j - nums[i])
    if split53Helper(nums, i + 1, j + nums[i]):
        return True
    else:
        return split53Helper(nums, i + 1, j - nums[i]) 
